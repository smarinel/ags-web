...
int almp3_do_layer2(struct frame *fr,unsigned char *pcm_sample,int *pcm_point)
{
  int clip=0;
  int i,j;
  int stereo = fr->stereo;
  ...
  for (i=0;i<SCALE_BLOCK;i++)
  {
    ...
    for (j=0;j<3;j++)
    {
      if(single >= 0)
      {
        clip += almp3_synth_1to1_mono(fraction[single][j],pcm_sample,pcm_point);
      }
      else {
          int p1 = *pcm_point;
          clip += almp3_synth_1to1(fraction[0][j],0,pcm_sample,&p1);
          clip += almp3_synth_1to1(fraction[1][j],1,pcm_sample,pcm_point);
      }
    }
  }
  return clip;
