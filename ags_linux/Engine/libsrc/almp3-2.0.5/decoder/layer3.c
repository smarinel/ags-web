 ...
 */
void almp3_init_layer3(int down_sample_sblimit)
{
  int i,j,k,l;
 ...
 * main layer3 handler
 */
int almp3_do_layer3(struct frame *fr, unsigned char *pcm_sample, int *pcm_point)
{
  int gr, ch, ss, clip = 0;
  ...
  for (gr=0;gr<granules;gr++)
  {
    ...
    for(ss=0;ss<SSLIMIT;ss++) {
      if(single >= 0) {
        clip += almp3_synth_1to1_mono(hybridOut[0][ss],pcm_sample,pcm_point);
      }
      else {
        int p1 = *pcm_point;
        clip += almp3_synth_1to1(hybridOut[0][ss],0,pcm_sample,&p1);
        clip += almp3_synth_1to1(hybridOut[1][ss],1,pcm_sample,pcm_point);
      }
    }
  }
